...
// @Success      200 {object} vo.ListPostsByUserIDResponseWrapper "热门帖子检索成功。" // <--- 修改
...
func (ctrl *HotPostController) GetHotPostsByCursor(c *gin.Context) {
	...
	// 4. 构造响应结构体 - 如注释所述，复用 ListPostsByUserIDResponse
	// 确保 vo.ListPostsByUserIDResponse 结构体匹配预期的输出 {posts, next_cursor}
	responseData := vo.ListPostsByUserIDResponse{ // 这里的业务逻辑仍然使用原始的 VO
...
func (ctrl *HotPostController) GetHotPostDetail(c *gin.Context) {
	...
	// 3. 将 UserID 存入上下文供服务层使用
	// 定义一个自定义类型作为上下文键以避免冲突
	type contextKey string
	const userIDKey contextKey = "UserID"
	ctx := context.WithValue(c.Request.Context(), userIDKey, userIDStr)

	// 4. 调用服务层获取热门帖子详情
	responseData, err := ctrl.postService.GetHotPostDetail(ctx, postID)
	if err != nil {
		// 考虑更具体的错误处理（例如，将服务错误映射到 404 未找到）
		// TODO: 假设服务层在找不到时返回特定的错误或(nil, nil)
		// if errors.Is(err, your_not_found_error) {
