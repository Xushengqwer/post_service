...
// @Param        official_tag query int false "按官方标签过滤 (例如, 0=无, 1=认证)" Enums(0, 1, 2, 3)
...
// @Router       /admin/posts/{id}/official-tag [put] // 改为 PUT，因为是更新操作
...
func (ctrl *PostAdminController) RegisterRoutes(group *gin.RouterGroup) {
	adminPosts := group.Group("/admin/posts") // 基础路径 /admin/posts
	{
		...
		// 如果需要，在此添加其他管理员路由，例如 DELETE /admin/posts/{id}
	}
}
