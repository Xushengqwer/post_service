...
type PostAdminService interface {
	...
	ListPostsByCondition(ctx context.Context, req *dto.ListPostsByConditionRequest) (*vo.ListPostsByConditionResponse, error)
	...
	// - 执行软删除操作。
	// - 记录管理员操作日志。
	DeletePostByAdmin(ctx context.Context, postID uint64, adminUserID string) error // 增加 adminUserID 用于日志记录
}
...
func (s *postAdminService) ListPostsByCondition(ctx context.Context, req *dto.ListPostsByConditionRequest) (*vo.ListPostsByConditionResponse, error) {
	// 直接调用仓库层进行查询。
	...
	// 将数据库实体转换为视图对象 (VO)。
	postResponses := make([]*vo.PostAdminResponse, 0, len(posts))
	for _, post := range posts {
		postResponses = append(postResponses, &vo.PostAdminResponse{
			ID:             post.ID,
			Title:          post.Title,
	...
	// 构造响应。
	response := &vo.ListPostsByConditionResponse{
		Posts: postResponses,
		Total: total,
	}
